...
public class MovieTest extends BaseTest {
    ...
    protected ClassLoader getClassLoader() {
        return MovieTest.class.getClassLoader();
    }
    ...
    public void asss() throws IOException {
        logger.info("开始执行添加数据,url:https://www.dy2018.com");
        ...
        if ("000000".equals(responseResult.getString("code"))) {
            logger.info("-------添加数据ok:" + requestParams.getTitle());
        } else {
            logger.info("-------添加数据error:" + requestParams.getTitle());
        }
    }
    ...
    public void add() {
        ...
        for (String movieUrl : moviesUrlSet) {
            ...
            logger.info("开始执行添加数据,url:" + movieUrl);
        ...
        logger.info("总计:" + totalCount + ",成功总数:" + okCount + ",失败总数" + failCount);
        logger.info("失败名称:" + sb.toString());
